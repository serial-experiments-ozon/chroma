...
pub(super) fn plugin(app: &mut App) {
  app
    ...
    .add_systems(OnEnter(Game::Splash), spawn_splash_screen);
  ...
  app.add_systems(
    ...
    (
      ...
      .run_if(in_state(Game::Splash)),
  );

  app
    .register_type::<SplashTimer>()
    .add_systems(OnEnter(Game::Splash), insert_splash_timer)
    .add_systems(OnExit(Game::Splash), remove_splash_timer)
    .add_systems(
      Update,
      (
  ...
  app.add_systems(
    ...
    enter_loading_screen
      .run_if(input_just_pressed(KeyCode::Escape).and(in_state(Game::Splash))),
  );
}
...
fn check_splash_timer(
  ...
  if timer.0.just_finished() {
    next_screen.set(Game::Loading);
  }
}

fn enter_loading_screen(mut next_screen: ResMut<NextState<Game>>) {
  next_screen.set(Game::Loading);
}
