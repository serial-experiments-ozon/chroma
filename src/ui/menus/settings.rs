...
use bevy::{audio::Volume, input::common_conditions::input_just_pressed};

use crate::{
  prelude::*,
  ui::{Menu, screens::Game, widget},
};
...
fn go_back_on_click(
  _: Trigger<Pointer<Click>>,
  screen: Res<State<Game>>,
  mut next_menu: ResMut<NextState<Menu>>,
) {
  next_menu.set(if screen.get() == &Game::Title {
    Menu::Main
  } else {
    Menu::Pause
  });
...
fn go_back(screen: Res<State<Game>>, mut next_menu: ResMut<NextState<Menu>>) {
  next_menu.set(if screen.get() == &Game::Title {
    Menu::Main
  } else {
    Menu::Pause
